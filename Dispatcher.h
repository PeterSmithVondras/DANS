...
class Dispatcher : public DStage {
public:
  ...
  // Purge will remove all instances of the Job linked to job_id that have
  // not already been scheduled. Returns OK on success. This function is often
  // overrode.
  Status Purge(JobId job_id) override;

protected:
  // PurgeDispatcher will remove all instances of the Job linked to job_id that
  // have not already been scheduled. Returns OK on success.
  Status PurgeDispatcher(JobId job_id);
