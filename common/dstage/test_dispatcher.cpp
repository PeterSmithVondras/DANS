...
int main() {
  ...
  Dispatcher<unsigned> disp(kMaxPrio);
  MultiQueue prio_qs(kMaxPrio);
  disp.LinkMultiQ(&prio_qs);

  JobIdFactory j_fact(0);
  std::list<Priority> purged;

  Job<unsigned> job(/*job_data=*/0, /*job_id=*/j_fact.CreateJobId(),
                    /*priority=*/0, /*requested_duplication=*/2);
  disp.Dispatch(job);
  assert(prio_qs.Purge(job.job_id).size() == kMaxPrio + 1);

  job = Job<unsigned>(/*job_data=*/0, /*job_id=*/j_fact.CreateJobId(),
